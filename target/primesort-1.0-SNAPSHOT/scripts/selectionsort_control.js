...
function selectionsort(data) {
    ...
    for(var i=0;i<n-1;i++){
        var min=i;
        text="Ustawianie elementu na pozycji "+i+" jako element o najmniejszej wartości.\n";
        for(var j=i+1;j<n;j++){

            console.log("Porównywanie " + idxdata[j][1] + " i " + idxdata[min][1]);
            stg++;
            stages[stg] = [];
            text+="Porównywanie elementu "+idxdata[j][1]+" z elementem minimalnym "+idxdata[min][1]+".\n";
            desc[stg]="";
            desc[stg]+=text;
            text="";
            ...
            if(idxdata[j][1]<idxdata[min][1]){
                text="Znaleziono element "+idxdata[j][1]+" o wartości mniejszej niż "+idxdata[min][1]+".\n";
                stages[stg][min][3]=1;
                min=j;
                stages[stg][min][3]=3;
            }
        ...
        if(min!=i){
            text+="Wstawianie najmniejszego elementu "+idxdata[min][1]+" na pozycję nr "+i+".\n";
            idxdata[i] = [idxdata[min], idxdata[min] = idxdata[i]][0];
            console.log("Zamiana " + idxdata[i][1] + " i " + idxdata[min][1]);
            stg++;
            stages[stg] = [];
            desc[stg]="";
            desc[stg]+=text;
...
async function selectionsort_vis(){
    var swapped=true;
    playing=true;
    $("#playicon").attr("xlink:href", "icons/bootstrap-icons.svg#pause-btn");
    ...
    for(stg=0;stg<stages.length&&playing==true;stg++) {
        ...
        if(!prgs_clicked){
            ...
            for(var i=0, j=0;i<dataset.length;i++){
                if(stages[s][i][3]==1||stages[s][i][3]==2||stages[s][i][3]==3) {
