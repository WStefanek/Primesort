...
function highlight(bar, mode){
    switch(mode){
        ...
        case 3: { //blue - current pivot
            bar.select("rect").transition()
                .ease(d3.easeCubic)
                .duration($("#speedslider").val()/2)
                .attr("style", "fill: rgb(70, 103, 250)");
            break;
        }
    }
}
...
$("#progressbar").click(function(){
    ...
    for(var j=0;j<dataset.length;j++){
        highlight(bars[stages[stg][j][0]], 0);
    }
    for(var j=0;j<dataset.length;j++){
        move(bars[stages[stg][j][0]], j);
        if(stages[stg][j][2]==1) highlight(bars[stages[stg][j][0]], 2);
        if(stages[stg][j][3]==1) highlight(bars[stages[stg][j][0]], 1);
        if(stages[stg][j][3]==2) highlight(bars[stages[stg][j][0]], 3);
    }
...
$("#playbtn").click(function(){
    if(!finished){
        ...
        for(var j=0;j<dataset.length;j++) {
            ...
            if (stages[0][j][2] == 1) highlight(bars[stages[0][j][0]], 1);
            else highlight(bars[stages[0][j][0]], 0);
        }
...
function partition(idxdata, min, max){
    //Selecting a pivot
    var pivot = idxdata[max][1];
    var i = min-1;
    var higher=false;
    var text="";
    stg++;
    stages[stg]=[];
    text="Ustawianie elementu na ostatniej pozycji jako pivot.\n";
    desc[stg]="";
    desc[stg]+=text;
    text="";
    for(var k=0;k<dataset.length;k++){
        if(stg>0){
            if(stages[stg-1][k][3]==2) stages[stg][k]=idxdata[k].concat([stages[stg-1][k][2], 2]);
            else stages[stg][k]=idxdata[k].concat([stages[stg-1][k][2], 0]);
        }else{
            stages[stg][k]=idxdata[k].concat([0, 0]);
        }
    }
    stages[stg][max][3]=2;
    console.log("Stage "+stg+" - setting a pivot: ");
    for(var k=0;k<dataset.length;k++){
        console.log(stages[stg][k]);
    }

    for(var j=min;j<=max-1;j++){
        //Comparing element with a pivot
        stg++;
        stages[stg]=[];
        text="Porównywanie elementu bieżącego "+idxdata[j][1]+" z pivotem "+pivot+".\n";
        desc[stg]="";
        desc[stg]+=text;
        text="";
        for(var k=0;k<dataset.length;k++){
        ...
        stages[stg][j][3]=1;
        console.log("Stage "+stg+" - comparing with a pivot: ");
        for(var k=0;k<dataset.length;k++){
            console.log(stages[stg][k]);
        }
        if(idxdata[j][1]>=pivot) higher=true;
        else{
            i++;
            if(higher){
                //Swapping elements
                idxdata[i]=[idxdata[j], idxdata[j]=idxdata[i]][0];
                stg++;
                stages[stg]=[];
                text="Przesuwanie bieżącego elementu na lewo od elementu "+idxdata[j][1]+" większego od pivota "+pivot+".\n";
                desc[stg]="";
                desc[stg]+=text;
                text="";
                for(var k=0;k<dataset.length;k++){
                ...
                console.log("Stage "+stg+" - swapping elements: ");
                for(var k=0;k<dataset.length;k++){
                    console.log(stages[stg][k]);
                }
            }
    ...
    idxdata[i+1]=[idxdata[max], idxdata[max]=idxdata[i+1]][0];
    stg++;
    stages[stg]=[];
    if(min==max){
        text="Pivot otoczony jest elementami ustawionymi na odpowiednim miejscu, więc również jest na odpowiednim miejscu.\n";
    }
    else{
        text="Mniejsze oraz większe elementy względem pivota są posegregowane. Ustawianie pivota "+pivot+" na jego odpowiednim miejscu.\n";
    }
    ...
    stages[stg][i+1][2]=1;
    stages[stg][max][3]=0;
    console.log("Stage "+stg+" - setting in its correct position: ");
    for(var k=0;k<dataset.length;k++){
        console.log(stages[stg][k]);
    }
    return (i+1);
...
function quicksort_rec(idxdata, min, max){
    if(min<=max){
        var pi=partition(idxdata, min, max);
        //Sortowanie rekursywnie
        quicksort_rec(idxdata, min, pi-1);
        quicksort_rec(idxdata, pi+1, max);
    }
}
...
function quicksort(data) {
    ...
    for (var i = 0; i < n; i++) {
        idxdata[i] = [i, data[i]]; //[index, value, color, comparison]
    }
    stg = -1;
    var text="";
...
async function quicksort_vis(){
    ...
    for(stg=0;stg<stages.length&&playing==true;stg++) {
        ...
        if(!prgs_clicked){
            //Podświetlanie pivota na kolor niebieski
            while(paused){
                await sleep(100);
            }
            for(var i=0;i<dataset.length;i++) {
                if (stages[s][i][3] == 2) {
                    highlight(bars[stages[s][i][0]], 3);
                }
            }
            //Podświetlanie pivota oraz elementu porównywanego na kolor żółty
            while(paused){
                await sleep(100);
            }
            for(var i=0;i<dataset.length;i++){
                if(stages[s][i][3]==1){
                    ...
                    for(var j=0;j<dataset.length;j++){
                        if(stages[s][j][3]==2) highlight(bars[stages[s][j][0]], 1);
                    }
                    swapped=false;
                }
            }
            //Jeżeli jest to etap zamiany, przesuń belki na odpowiednie miejsca. Jeżeli nie jest to etap zamiany, przywróć oryginalne kolory belek
            if(swapped){
                while(paused){
                    await sleep(100);
                }
                ...
                for(var i=0;i<dataset.length;i++){
                    if(stages[s][i][3]==1) highlight(bars[stages[s][i][0]], 0);
                    if(stages[s][i][3]==2){
                        highlight(bars[stages[s][i][0]], 3);
                    }
                }
            ...
            //Jeżeli któraś z belek jest już posortowana, zamień jej kolor na zielony
            while(paused){
                await sleep(100);
            }
            for(var j=0;j<dataset.length;j++){
                if(stages[s][j][2]==1) highlight(bars[stages[s][j][0]], 2);
            }
        }
        prgs_clicked=false;
        //Jeżeli jest to przedostatni etap, zamień kolory wszystkich belek na zielony
